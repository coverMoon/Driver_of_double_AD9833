...
******************************************************************************
  * @file           : AD9833_Soft.c
  * @brief          : AD9833 DDS波形发生器驱动库 (纯软件模拟SPI)
  * @author         : 秦泽宇
  * @version        : v1.1
  * @date           : 2025-07-18
  *
  ...
  * @attention
  *
  * 本驱动库是为 AD9833 DDS 波形发生器芯片编写，并适配了STM32的硬件
  * 抽象层 (HAL) 库。它完全通过软件模拟SPI时序进行通信，不依赖于硬件
  * SPI外设，因此具有更高的引脚灵活性。
  *
  * 主要功能包括：
  * - 支持单通道或双通道独立工作模式。
  ...
  *
  * 使用方法：
  * 1. 在 `main.h` 或相关头文件中包含 `"AD9833_Soft.h"`。
  * 2. 在 CubeMX 中将用于软件SPI的引脚 (SCLK, MOSI) 和片选 (CS)
  * 配置为GPIO推挽输出模式，同时添加User label (如AD9833_SCLK、
  * AD9833_MOSI、AD9833_CS1、AD9833_CS2)
  * 3. 创建一个 `AD9833_InitTypedef` 结构体变量并填充所需参数
  * （工作模式、波形、频率、相位等）。
  * 4. 调用 `AD9833_Cmd()` 或 `AD9833_Cmd_Sync()` 函数来初始化并启动芯片。
  *
  ******************************************************************************
  */
