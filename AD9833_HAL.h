#ifndef _AD9833_HAL_H
#define _AD9833_HAL_H
...
typedef struct
{
    waveType wave;   // 波形种类 (使用 waveType 枚举)
    double freq;        // 频率 (Hz)
    double phase;       // 相位 (弧度, 0 到 2*PI)
    uint8_t freqReg;    // 使用的频率寄存器 (0 或 1)
    uint8_t phaseReg;   // 使用的相位寄存器 (0 或 1)
} DDS_InitTypedef;

/**
  * @brief AD9833初始化结构体，若只需要一路输出，另一通道可直接初始化为0
  */
typedef struct
{
    SPI_HandleTypeDef* hspi;  // 指向SPI句柄的指针
    workStatus status;       // 工作状态 (使用 workStatus 枚举)
    DDS_InitTypedef AD_CS1;   // 通道一输出参数
    DDS_InitTypedef AD_CS2;   // 通道二输出参数
} AD9833_InitTypedef;
